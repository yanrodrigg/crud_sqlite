...
# cursor.execute(sql_update, ("Ipad Pro", "Ipad 11 PRO, 128gb, Cinza", 1500.00, "ipad_pro.jpg", ))  #execução do comando sql que está escrito na variável sql_insert. Como passamos um ? para indicar a inclusão de um argumento, precisamos passar os valores em forma de tupla.
# conn.commit()#salvando as alterações no banco de dados
# conn.close()#fechando a conexão com o banco de dados. É MUITO importante não esquecer de fechar a conexão com o banco para evitar erros
